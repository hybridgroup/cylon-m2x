 ...
 *
 **/
Driver.prototype.subscribe = function(deviceId, stream, options, callback) {
 ...
 *
 **/
Driver.prototype.publish = function(deviceId, stream, value, callback) {
  var unique = deviceId + ":" + stream + ":publish";

  if (this.connection.listeners(unique).length === 0) {
    this.defineDriverEvent(stream + ":publish");
    this.defineDriverEvent(unique);
  }
